...
namespace United.Mobile.ViewResSeatMap.Api
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddTransient<IShoppingSessionHelper, ShoppingSessionHelper>();
            services.AddTransient<IViewResSeatMapBusiness, ViewResSeatMapBusiness>();
        ...
        public void ConfigureContainer(ContainerBuilder container)
        {
            ...
            container.Register(c => new ResilientClient(Configuration.GetSection("dpTokenConfig").Get<ResilientClientOpitons>())).Keyed<IResilientClient>("dpTokenServiceConfigKey");
            container.RegisterType<DPTokenService>().As<IDPTokenService>().WithAttributeFiltering();
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, IApplicationEnricher applicationEnricher, IFeatureSettings featureSettings, IHostApplicationLifetime applicationLifetime)
        {
            ...
            app.MapWhen(context => string.IsNullOrEmpty(context.Request.Path) || string.Equals("/", context.Request.Path), appBuilder =>
            {
                ...
                {
                    await context.Response.WriteAsync("Welcome from ViewRes SeatMap Microservice");
                });
            });
            ...
            app.UseStaticFiles(new StaticFileOptions
            {
                ...
                RequestPath = "/viewresseatmapservice/images"
            });
            app.UseDirectoryBrowser(new DirectoryBrowserOptions()
            {
                FileProvider = new PhysicalFileProvider(
                Path.Combine(Directory.GetCurrentDirectory(), "images")),
                RequestPath = new PathString("/viewresseatmapservice/images")
            });
        ...
        private async Task OnStart(IFeatureSettings featureSettings)
        {
            try
            {
                await featureSettings.LoadFeatureSettings(United.Mobile.Model.Common.ServiceNames.VIEWRESSEATMAP.ToString());
            }
            catch (Exception) { }
        ...
        private async Task OnShutDown(IFeatureSettings featureSettings)
        {
            try
            {
                await featureSettings.DeleteContainerIPAdress(United.Mobile.Model.Common.ServiceNames.VIEWRESSEATMAP.ToString(), StaticDataLoader._ipAddress);
            }
            catch { }
        }
